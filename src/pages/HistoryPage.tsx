...
export function HistoryPage() {
  ...
  return <div className="page">
    ...
    <Card>
      <div className="filter-bar"><div className="search-field"><Search aria-hidden="true" /><input value={query} onChange={(event) => setParams(event.target.value ? { q: event.target.value } : {})} placeholder={t('searchTitles')} aria-label={t('searchTitles')} /></div><label className="select-with-icon"><Filter size={16} aria-hidden="true" /><select value={status} onChange={(event) => setStatus(event.target.value)} aria-label={t('allStatuses')}><option value="all">{t('allStatuses')}</option><option value="ready">{t('ready')}</option><option value="processing">{t('processing')}</option><option value="failed">{t('failed')}</option></select></label></div>
