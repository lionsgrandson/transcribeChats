...
export function CalendarPage() {
  ...
  return <div className="page">
    ...
    <Card className="calendar-card">
      <div className="calendar-toolbar"><div className="calendar-nav"><button type="button" className="icon-button" onClick={() => setCursor(addMonths(cursor, -1))} aria-label="Previous month"><ArrowLeft /></button><h2 aria-live="polite">{format(cursor, 'MMMM yyyy')}</h2><button type="button" className="icon-button" onClick={() => setCursor(addMonths(cursor, 1))} aria-label="Next month"><ArrowRight /></button><Button variant="ghost" onClick={() => setCursor(new Date())}>{t('today')}</Button></div><div className="segmented-control compact" role="group" aria-label="Calendar view"><button type="button" aria-pressed={view === 'month'} className={view === 'month' ? 'active' : ''} onClick={() => setView('month')}><CalendarDays size={15} />{t('month')}</button><button type="button" aria-pressed={view === 'agenda'} className={view === 'agenda' ? 'active' : ''} onClick={() => setView('agenda')}><List size={15} />{t('agenda')}</button></div></div>
