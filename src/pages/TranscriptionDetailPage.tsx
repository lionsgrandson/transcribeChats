...
function ItemRow({ item, transcriptionTitle, selected, onSelect, onUpdate, onDelete, onSource, onPlaySource }: { item: ExtractedItem; transcriptionTitle: string; selected: boolean; onSelect: (selected: boolean) => void; onUpdate: (patch: Partial<ExtractedItem>) => Promise<void>; onDelete: () => Promise<void>; onSource: (segmentId: string) => void; onPlaySource: (segmentId: string) => void }) {
  const { t } = useTranslation();
  ...
  return <div className="page detail-page">
    ...
    <Card className="detail-content" id="transcription-tabpanel" role="tabpanel" aria-labelledby={`transcription-tab-${tab}`}>
      ...
      {tab === 'tasks' && <div>
        ...
        {items.filter((item) => item.kind === 'task' || item.kind === 'event').length ? <div className="item-list">{items.filter((item) => item.kind === 'task' || item.kind === 'event').map((item) => <ItemRow key={item.id} item={item} transcriptionTitle={transcription.title} selected={selectedTaskIds.has(item.id)} onSelect={(value) => toggleSelectedTask(item.id, value)} onUpdate={(patch) => updateDetailItem(item.id, patch)} onDelete={async () => { toggleSelectedTask(item.id, false); await store.deleteItem(item.id); }} onSource={showSource} onPlaySource={playSource} />)}</div> : <EmptyState title={t('noTasks')} body="No explicit commitments, direct requests, or planned timeline events were found." />}
      </div>}
