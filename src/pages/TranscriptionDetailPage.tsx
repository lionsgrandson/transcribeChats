...
export function TranscriptionDetailPage() {
  ...
  return <div className="page detail-page">
    ...
    <Card className="detail-content">
      ...
      {tab === 'tasks' && <div><div className="section-heading"><div><h2>{t('tasks')} & {t('events')}</h2><p>Only explicit commitments and meeting proposals appear here. Nothing reaches the calendar until you accept it and add a date.</p></div><Button variant="secondary" onClick={() => void store.addItem(id, { kind: 'task', title: t('addTask') })}><Plus size={16} />{t('addTask')}</Button></div>{items.filter((item) => item.kind === 'task' || item.kind === 'event').length ? <div className="item-list">{items.filter((item) => item.kind === 'task' || item.kind === 'event').map((item) => <ItemRow key={item.id} item={item} transcriptionTitle={transcription.title} onUpdate={(patch) => store.updateItem(item.id, patch)} onDelete={() => store.deleteItem(item.id)} onSource={showSource} onPlaySource={playSource} />)}</div> : <EmptyState title={t('noTasks')} body="No explicit commitments or meeting proposals were found. Add one manually or send the transcript to Ollama." />}</div>}
