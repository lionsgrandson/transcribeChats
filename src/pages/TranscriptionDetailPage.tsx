...
function EditableSegment({ segment, onSave }: { segment: TranscriptSegment; onSave: (text: string, speaker: string) => Promise<void> }) {
  const { t } = useTranslation();
  const [editing, setEditing] = useState(false);
  const [text, setText] = useState(segment.text);
  const [speaker, setSpeaker] = useState(segment.speakerLabel);
  return <article className="segment-row" id={`segment-${segment.id}`}>
    <button className="timestamp-button" title="Play from this timestamp"><Play size={13} />{formatTimestamp(segment.startMs)}</button>
...
function ItemRow({ item, onUpdate }: { item: ExtractedItem; onUpdate: (patch: Partial<ExtractedItem>) => Promise<void> }) {
  const { t } = useTranslation();
  ...
  return <div className={`item-row ${item.status === 'completed' ? 'is-completed' : ''}`}>
    ...
    {item.sourceSegmentIds[0] && <a className="evidence-link" href={`#segment-${item.sourceSegmentIds[0]}`}>{t('source')}</a>}
  </div>;
}
...
export function TranscriptionDetailPage() {
  ...
  return <div className="page detail-page">
    ...
    <Card className="detail-content">
      {tab === 'transcript' && (segments.length ? <div className="transcript-view"><div className="transcript-toolbar"><div><strong>{segments.length} segments</strong><span>{new Set(segments.map((segment) => segment.speakerLabel)).size} {t('speakers').toLowerCase()}</span></div></div>{segments.map((segment) => <EditableSegment key={segment.id} segment={segment} onSave={(text, speaker) => store.updateSegment(segment.id, text, speaker)} />)}</div> : <EmptyState title="Transcript not available yet" body={transcription.status === 'failed' ? 'Retry processing when the local worker is running.' : 'Segments will appear here while processing completes.'} />)}
      {tab === 'tasks' && <div><div className="section-heading"><div><h2>{t('tasks')} & {t('events')}</h2><p>{t('reviewExtracted')}</p></div><Button variant="secondary" onClick={() => void store.addItem(id, { kind: 'task', title: t('addTask') })}><Plus size={16} />{t('addTask')}</Button></div>{items.filter((item) => item.kind === 'task' || item.kind === 'event').length ? <div className="item-list">{items.filter((item) => item.kind === 'task' || item.kind === 'event').map((item) => <ItemRow key={item.id} item={item} onUpdate={(patch) => store.updateItem(item.id, patch)} />)}</div> : <EmptyState title={t('noTasks')} body={t('noTasksBody')} />}</div>}
