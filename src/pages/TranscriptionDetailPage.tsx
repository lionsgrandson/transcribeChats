...
export function TranscriptionDetailPage() {
  ...
  const extractedNotes = items.filter((item) => item.kind === 'note' || item.kind === 'takeaway' || item.kind === 'summary');
  ...
  return <div className="page detail-page">
    ...
    <div className="detail-tabs" role="tablist">{tabs.map(({ id: value, label, icon: Icon }) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}><Icon size={17} />{label}{value === 'tasks' && items.filter((item) => (item.kind === 'task' || item.kind === 'event') && item.status === 'needs_review').length > 0 && <span className="tab-count">{items.filter((item) => (item.kind === 'task' || item.kind === 'event') && item.status === 'needs_review').length}</span>}</button>)}</div>
    ...
    <Card className="detail-content">
      ...
      {tab === 'tasks' && <div><div className="section-heading"><div><h2>{t('tasks')} & {t('events')}</h2><p>Tasks are explicit commitments or requests. Planned meetings and dated occurrences are events/timeline items, not tasks.</p></div><Button variant="secondary" onClick={() => void store.addItem(id, { kind: 'task', title: t('addTask') })}><Plus size={16} />{t('addTask')}</Button></div>{items.filter((item) => item.kind === 'task' || item.kind === 'event').length ? <div className="item-list">{items.filter((item) => item.kind === 'task' || item.kind === 'event').map((item) => <ItemRow key={item.id} item={item} transcriptionTitle={transcription.title} onUpdate={(patch) => store.updateItem(item.id, patch)} onDelete={() => store.deleteItem(item.id)} onSource={showSource} onPlaySource={playSource} />)}</div> : <EmptyState title={t('noTasks')} body="No explicit commitments, direct requests, or planned timeline events were found." />}</div>}
