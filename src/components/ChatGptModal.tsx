...
export function ChatGptModal({ open, onClose, transcription, segments, items, onImport }: { open: boolean; onClose: () => void; transcription: Transcription; segments: TranscriptSegment[]; items: ExtractedItem[]; onImport: (values: ReturnType<typeof previewImport>['valid']) => Promise<void> }) {
  ...
  const transcript = useMemo(() => segments.map((segment) => `[${formatTimestamp(segment.startMs)}] ${segment.speakerLabel}: ${segment.text}`).join('\n'), [segments]);
  const taskPrompt = `\n\nConvert this transcript into JSON using exactly this shape:\n{"items":[{"kind":"task|event|note|takeaway|summary","title":"...","body":"...","status":"needs_review","priority":"none|low|medium|high|urgent","assignee":null,"startsAt":null,"endsAt":null,"dueAt":null,"reminderAt":null,"tags":[],"sourceSegmentIds":[],"confidence":0.0,"uncertaintyReason":null,"confirmed":false}]}\nConversation date: ${transcription.recordedAt}. Do not invent an assignee or date. Use ISO 8601 timestamps. Return only a fenced JSON object.`;
