...
export function ChatGptModal({ open, onClose, transcription, segments, items, onImport }: { open: boolean; onClose: () => void; transcription: Transcription; segments: TranscriptSegment[]; items: ExtractedItem[]; onImport: (values: ReturnType<typeof previewImport>['valid']) => Promise<void> }) {
  ...
  const taskPrompt = `\n\nAnalyze this transcript into JSON using exactly this shape:\n{"items":[{"kind":"task|event|note|takeaway|summary","title":"...","body":"...","status":"needs_review","priority":"none|low|medium|high|urgent","assignee":null,"startsAt":null,"endsAt":null,"dueAt":null,"reminderAt":null,"tags":[],"sourceSegmentIds":["exact-segment-id"],"confidence":0.0,"uncertaintyReason":null,"confirmed":false}]}\nConversation date: ${transcription.recordedAt}. A task requires an explicit commitment or direct request. Facts, status updates, availability, birthdays, blockers, risks, preferences, and useful reminders are notes, not tasks. Preserve the exact segment_id in sourceSegmentIds for every item so the app can jump to and play the source timestamp. Do not invent an assignee or date. Use ISO 8601 timestamps. Return only a fenced JSON object.`;
