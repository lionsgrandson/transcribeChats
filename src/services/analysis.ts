...
const explicitEnglishTask = new RegExp(`\\b(?:(?:i|we)\\s+(?:will|shall|must|have\\s+to|am\\s+going\\s+to|are\\s+going\\s+to|committed\\s+to)|you\\s+(?:must|have\\s+to)|(?:please|can\\s+you|could\\s+you))\\s+${actionVerbs}\\b`, 'i');
