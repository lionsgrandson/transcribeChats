...
def _transcribe_sync(path: Path, language_mode: str) -> tuple[list[Segment], list[str], int | None]:
    ...
    raw_segments, info = model.transcribe(str(path), language=language, beam_size=5, vad_filter=True, word_timestamps=True, condition_on_previous_text=True)
    ...
    for index, value in enumerate(raw_segments):
        ...
        segments.append(Segment(id=str(uuid.uuid4()), sequence_no=index, start_ms=round(value.start * 1000), end_ms=round(value.end * 1000), text=value.text.strip(), language=info.language or language or "auto", confidence=confidence))
    duration = round(info.duration * 1000) if getattr(info, "duration", None) else (segments[-1].end_ms if segments else None)
    return segments, [info.language] if info.language else ([language] if language else []), duration
