...
Open **Settings → CRM connection**, choose CodeCrafter CRM, CreativeCRM, or another compatible webhook, and paste the webhook endpoint and API token generated by that CRM. Use **Test CRM connection** before sending tasks. Single-task and bulk actions then post directly to the selected CRM instead of opening a hard-coded CodeCrafter URL. The endpoint and token stay in this app's local IndexedDB settings.
